...
import { initialMessages, initialNotes } from "@/data/instagram";
...
const DEFAULT_ART_PHOTO = "data:image/svg+xml;charset=UTF-8,%3Csvg xmlns='http://www.w3.org/2000/svg' width='100' height='100'%3E%3Crect width='100' height='100' fill='%23666'/%3E%3Ccircle cx='50' cy='40' r='20' fill='%23ccc'/%3E%3Cpath d='M20 100 Q 50 60 80 100' fill='%23ccc'/%3E%3C/svg%3E";
...
export default function IGMessages() {
    ...
    const profilePhoto = usePhotoUpload(DEFAULT_AVATAR);

    // === SETTING STATES ===
    const [headerUsername, setHeaderUsername] = useState("aninketos");
        ...
        <div className={`bg-[#09090b] min-h-screen w-full flex flex-col xl:flex-row items-center justify-start xl:justify-center ${isStandalone ? 'p-0 gap-0' : 'py-0 xl:py-10 px-0 xl:px-4 gap-0 xl:gap-10'} font-sans overflow-x-hidden`}>
            ...
            <div className={`w-full xl:w-auto flex justify-center ${mobileTab === "edit" ? 'flex relative' : 'absolute opacity-0 pointer-events-none -z-50 xl:relative xl:opacity-100 xl:pointer-events-auto xl:z-auto xl:flex'}`}>
                <MessagesControlPanel
                    header={{
                        ...
                        setProfileImage: profilePhoto.setImage,
                    }}
                    statusBar={{
                        ...
                        diWaveColor: deviceSettings.diWaveColor,
                        diPhoto: diPhoto,

                        setTimeText: (val) => setDeviceSettings(prev => ({ ...prev, timeText: val })),
                        setBatteryLevel: (val) => setDeviceSettings(prev => ({ ...prev, batteryLevel: val })), // 👈 NAH DI SINI TARUHNYA!
                    ...
                    notesLayer={{
                        notes: notesLayer.items,
                        activeId: notesLayer.activeId,
                        setActiveId: notesLayer.setActiveId,
                        updateNote: notesLayer.updateItem,
                        addNote: handleAddNote,
                        removeNote: notesLayer.removeItem,
                    }}
                    msgsLayer={{
                        messages: msgsLayer.items,
                        activeId: msgsLayer.activeId,
                        setActiveId: msgsLayer.setActiveId,
                        updateMsg: msgsLayer.updateItem,
                        addMsg: handleAddMsg,
                        removeMsg: msgsLayer.removeItem,
                    }}
                    footer={{ showBadge, setShowBadge }}
            ...
            <div className={`w-full xl:w-auto flex justify-center py-6 xl:py-0 ${mobileTab === "preview" ? 'flex relative' : 'absolute opacity-0 pointer-events-none -z-50 xl:relative xl:opacity-100 xl:pointer-events-auto xl:z-auto xl:flex'}`}>
                <div
                    ...
                    className={
                        isStandalone
                            ...
                            : "w-[393px] max-w-[100vw] h-[852px] bg-white flex flex-col overflow-hidden relative shadow-2xl ring-1 ring-white/10 sm:rounded-[40px] shrink-0"
                    }
                >
                    ...
                    <div className="w-full h-[53px] border-t-[0.5px] border-gray-200 bg-white flex justify-around items-center shrink-0 mt-auto px-2 z-20">
                        ...
                        <button className="relative p-2 cursor-pointer hover:opacity-70 transition-opacity">
                            ...
                            {showBadge && <div className="absolute bottom-2 right-1.5 bg-red-600 w-2 h-2 rounded-full border-[0.5px] border-white" />}
                        </button>
                        <button className="p-2 cursor-pointer hover:opacity-70 transition-opacity">
                        ...
                        <button className="p-2 cursor-pointer hover:opacity-70 transition-opacity">
                            <div className="w-7 h-7 rounded-full bg-gray-200 overflow-hidden border border-gray-300">
                                <img src={profilePhoto.image} alt="Profile" className="w-full h-full object-cover" />
                            </div>
                        </button>
                    </div>
                ...
                {replyingNote && (
                    <div className="absolute inset-0 z-[100] flex flex-col justify-end overflow-hidden">
                        ...
                        <div className="relative w-full bg-white rounded-t-[32px] flex flex-col items-center pt-3 pb-4 shadow-2xl animate-in slide-in-from-bottom-10 duration-200 z-10">
                            ...
                            {/* Panggil Komponen Note Detail Item */}
                        ...
                        {/* Keyboard iOS sesuai CSS Figma lo */}
                ...
                <div className="xl:hidden fixed bottom-0 left-0 right-0 p-5 bg-[#09090b]/90 backdrop-blur-md border-t border-white/10 flex gap-3 z-[100] pb-10">
                    <button onClick={() => downloadImage(phoneFrameRef, "jpg", `dm-${headerUsername || "view"}`)} className="flex-1 bg-[#27272a] hover:bg-[#3f3f46] text-white py-3 rounded-xl text-sm font-bold transition-all border border-[#3f3f46]">Save JPG</button>
                    <button onClick={() => downloadImage(phoneFrameRef, "png", `dm-${headerUsername || "view"}`)} className="flex-[2] bg-indigo-600 hover:bg-indigo-500 text-white py-3 rounded-xl text-sm font-bold transition-all flex justify-center items-center gap-2 shadow-lg shadow-indigo-900/30">
