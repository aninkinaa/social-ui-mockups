        ...
        <div className="w-full xl:w-[480px] bg-[#18181b] xl:border-l border-[#27272a] flex flex-col h-auto min-h-screen xl:h-screen relative shrink-0 z-20">
            ...
            <div className="flex-1 p-6 overflow-y-auto custom-scrollbar flex flex-col gap-8 pb-32 relative z-10">
                ...
                <Section title="Comments Settings" icon={<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>}>
                    ...
                    <div className="border-t border-[#3f3f46]/50 pt-5 mt-2 flex flex-col gap-4">
                        <Toggle 
                            label="Show Comments Sheet" 
                            subLabel="Bottom sheet with comment list" 
                        ...
                        {commentSheet.isOpen && (
                            <div className="flex flex-col gap-4">
                                <div className="flex gap-4 items-start bg-black/20 p-3 rounded-xl border border-white/5 mt-1">
                                    <div className="flex flex-col gap-1.5 items-center shrink-0">
                                        <label className="text-[10px] font-bold text-brand uppercase">POV Viewer</label>
                                        <AvatarInput className="w-10 h-10 shadow-sm border border-brand/50" value={viewerPhoto.image} onChange={(base64) => viewerPhoto.setImage(base64)} />
                                    </div>
                                    <div className="flex-1 text-[10px] text-neutral-400 leading-snug pt-2">
                                        This avatar will be used in the comment input field at the bottom of the sheet, representing you.
                                    </div>
                                </div>

                                <button type="button" onClick={commentSheet.addComment} className="bg-brand/10 border border-brand/20 hover:bg-brand/20 w-full py-2.5 rounded-xl text-[12px] font-bold text-brand transition-all flex items-center justify-center gap-2">
                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" /></svg> 
                                    Add Sheet Comment
                                </button>
                                ...
                                <div className="flex flex-col gap-2">
                                    {commentSheet.comments.map((c, index) => (
                                        <div 
                                            ...
                                            className={`bg-[#18181b] p-3 rounded-xl border transition-colors ${commentSheet.activeId === c.id ? 'border-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.2)]' : 'border-[#3f3f46]/50'} ${dragIndex === index ? 'opacity-50 border-dashed' : ''}`}
                                        >
                                            ...
                                            <div className="flex justify-between items-center cursor-pointer" onClick={() => commentSheet.setActiveId(commentSheet.activeId === c.id ? null : c.id)}>
                                                <div className="flex items-center gap-2 overflow-hidden flex-1 pr-2">
                                                    ...
                                                    <span className="text-[13px] font-medium text-white truncate">{c.username || 'Username'} - {c.text}</span>
                                                </div>
                                                <button onClick={(e) => { e.stopPropagation(); commentSheet.removeComment(c.id); }} className="text-neutral-500 hover:text-red-400 p-1.5 hover:bg-[#18181b] rounded-md transition-colors shrink-0 bg-[#27272a]">
                                                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg>
                                                </button>
                                            </div>
                                            ...
                                            {commentSheet.activeId === c.id && (
                                                <div className="mt-4 flex flex-col gap-4 pt-4 border-t border-[#3f3f46]/50">
                                                    <div className="flex gap-3 items-start bg-black/20 p-3 rounded-xl border border-white/5">
                                                        <AvatarInput className="w-10 h-10 shrink-0 shadow-sm mt-0.5" value={c.avatar} onChange={(base64) => commentSheet.updateComment(c.id, 'avatar', base64)} />
                                                        <div className="flex-1 flex flex-col gap-2 min-w-0">
                                                            <Input value={c.username} onChange={(e) => commentSheet.updateComment(c.id, 'username', e.target.value)} placeholder="Username" className="h-8 text-[12px]" />
                                                            <textarea value={c.text} onChange={(e) => commentSheet.updateComment(c.id, 'text', e.target.value)} placeholder="Comment Text..." className="bg-[#18181b] border border-[#3f3f46] text-white text-[12px] rounded-lg px-2 py-1.5 outline-none focus:border-indigo-500 w-full min-h-[44px] resize-y custom-scrollbar" />
                                                        </div>
                                                    </div>
