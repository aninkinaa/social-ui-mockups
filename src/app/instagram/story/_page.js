...
export default function IGStory() {
  ...
  const [mobileTab, setMobileTab] = useState("preview"); // "preview" | "edit"
  ...
  const downloadImage = async (format = "png") => {
    ...
    try {
      const options = { quality: 1.0, pixelRatio: 3, style: { borderRadius: "0px", boxShadow: "none", outline: "none", transform: "scale(1)", margin: "0", WebkitFontSmoothing: "antialiased", MozOsxFontSmoothing: "grayscale" } };
  ...
  return (
    <div className={`bg-[#09090b] min-h-screen w-full flex flex-col xl:flex-row items-center justify-start xl:justify-center ${isStandalone ? 'p-0 gap-0' : 'py-0 xl:py-10 px-0 xl:px-4 gap-0 xl:gap-10'} font-sans overflow-x-hidden`}>
      ...
      {/* === CONTROL PANEL === */}
      <div className={`w-full xl:w-auto flex justify-center ${mobileTab === "edit" ? 'flex' : 'hidden xl:flex'}`}>
        ...
        {/* Sticky Export Area */}
        <div className="fixed xl:absolute bottom-0 left-0 right-0 p-5 bg-[#18181b]/90 backdrop-blur-md border-t border-[#27272a] flex gap-3 z-30 shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
      ...
      {/* === PHONE FRAME === */}
      <div className={`w-full xl:w-auto flex justify-center py-6 xl:py-0 ${mobileTab === "preview" ? 'flex' : 'hidden xl:flex'}`}>
        <div 
          ref={phoneFrameRef} 
          className={
            isStandalone 
            ? "w-full h-[100dvh] bg-white flex flex-col overflow-hidden" 
            : "w-[393px] max-w-[100vw] h-[852px] bg-[#0F0F0F] flex flex-col overflow-hidden relative shadow-2xl ring-1 ring-white/10 sm:rounded-[40px] shrink-0"
          }
        >
