...
export default function IGprofile() {
    ...
    const [bioProps, setBioProps] = useState(initialBio);
        ...
        <div className="bg-[#09090b] min-h-screen w-full flex flex-col xl:flex-row font-sans overflow-x-hidden relative">
                ...
                >
                    <div ref={phoneFrameRef} className="w-full h-full bg-white flex flex-col relative overflow-hidden">
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col pb-4">
                            ...
                            {/* Followed by */}
                            {followedBy.show && activeFollowedUsers.length > 0 && (
                                <div className="mx-4 py-3 flex items-center gap-3">
                                    <div className="flex items-center shrink-0">
                                        {activeFollowedUsers.map((user, idx) => (
                                            <img
                                                key={user.id}
                                                src={user.avatar || DEFAULT_AVATAR}
                                                className={`w-9 h-9 rounded-full border-[1.5px] border-white relative object-cover ${idx === 0 ? 'z-30' : idx === 1 ? 'z-20 -ml-2.5' : 'z-10 -ml-2.5'
                                                    }`}
                                                alt={`Follower Avatar ${idx + 1}`}
                                            />
                                        ))}
                                    </div>

                                    <div className="text-black text-[14px] leading-snug line-clamp-2">
                                        <span>Followed by </span>
                                        <span className="font-semibold">
                                            {activeFollowedNames.map((user, idx, arr) => {
                                                let separator = "";
                                                const isLastItem = idx === arr.length - 1;

                                                if (!isLastItem) {
                                                    if (idx === arr.length - 2 && !followedBy.othersCount) {
                                                        separator = <span className="font-normal"> and </span>;
                                                    } else {
                                                        separator = ", ";
                                                    }
                                                } else if (followedBy.othersCount) {
                                                    separator = <span className="font-normal"> and </span>;
                                                }
                                            ...
                                            {followedBy.othersCount && (
                                                <>{followedBy.othersCount} others</>
                                            )}
                                        </span>
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-3 gap-[1px] w-full bg-white pb-[1px]">
                                {postsLayer.items.map((post) => (
                                    <div key={post.id} className="relative bg-gray-200 aspect-[130/171]">
                                        {post.image && (
            ...
            <div className={`w-full xl:w-[480px] shrink-0 xl:h-screen xl:sticky xl:top-0 z-40 ${mobileTab === "edit" ? 'block' : 'hidden xl:block'}`}>
                <ProfileControlPanel
                    ...
                    followedBy={followedBy} setFollowedBy={setFollowedBy}
